...
export class TextosComponent implements OnInit {
  ...
  ngOnInit() {
    this._service.getHistorias().subscribe(
      res => {
  ...
  updateText() {
    ...
    if (this.btnText == "Agregar") {
      ...
      console.log(obj);
      this._service.agregarHistoria(obj).subscribe(
        res => {
          console.log(res);
      ...
      console.log(obj);
      this._service.actualizarHistoria(obj).subscribe(
        res => {
          console.log(res);
  ...
  borrarHistoria(item) {
    ...
    console.log("--------------");
    var pos = this.arrayHistorias.indexOf(
      this.arrayHistorias.filter(i => {
        return i.id == item.id;
      })[0]
    );
    this._service.borrarHistoria(deleted).subscribe(
      res => {
        console.log(res);
